...
Inputdialog=dlgcreatedialog("Direct calution of the Niggli Cell (Pro)", Inputitems)
...
taggroup epsLabel=dlgcreatelabel("eps (A)")
taggroup eps1=dlgcreaterealfield(0.05,6,2)
taggroup eps2=dlgcreaterealfield(0.05,6,2)
taggroup eps3=dlgcreaterealfield(0.05,6,2)
taggroup epsGroup=dlggroupitems(epsLabel, eps1,eps2,eps3).dlgtablelayout(4,1,0)
...
taggroup ParameterGroup=dlggroupitems(CellGroup, epsGroup, NGroup).dlgtablelayout(3,1,0)
...
epsval1=eps1.dlggetvalue()	
epsval2=eps2.dlggetvalue()	
epsval3=eps3.dlggetvalue()	
...
result("\n----------------------------------------------- Direct calution of the Niggli Cell (Pro) -----------------------------------------------\n")
result("The input cell: ("+a0+", "+b0+", "+c0+", "+d0+", "+e0+", "+f0+" | "+Vrec+"), eps 1-3: ("+epsval1+", "+epsval2+", "+epsval3+"), Search Range: "+size+"\n")
...
number t10,t20=b0,t30=c0  //three temp parameters
t10=imguv.min(x,y)  
...
if((v1*w2-v2*w1)!=0||-(w1*u2-w2*u1)!=0||(u1*v2-u2*v1)!=0)   //Make sure  two vectors t1 and t2 are non-collinear，010,020,0-20; 111,222
{
if(t2>t20+epsval2&&no>0)
{
imguv=imguvCopy1  //return t1
imgw = imgWCopy1 
no2=0
...
if(V>1e-6)
{
if(t3>t30+epsval3&&no>0)
{
imguv=imguvCopy2 //return t2
imgw = imgWCopy2 
no3=0
